        ...
        <div class="p-6 text-gray-900">
            <div class="mb-6">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('admin.commandes.index', ['status' => '']) }}" class="px-4 py-2 rounded-full text-sm {{ !request('status') ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }}">
                        Toutes
                    </a>
                    <a href="{{ route('admin.commandes.index', ['status' => 'en_attente']) }}" class="px-4 py-2 rounded-full text-sm {{ request('status') === 'en_attente' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }}">
                        En attente
                    </a>
                    <a href="{{ route('admin.commandes.index', ['status' => 'traite']) }}" class="px-4 py-2 rounded-full text-sm {{ request('status') === 'traite' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }}">
                        Traitées
                    </a>
                    <a href="{{ route('admin.commandes.index', ['status' => 'expedie']) }}" class="px-4 py-2 rounded-full text-sm {{ request('status') === 'expedie' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }}">
                        Expédiées
                    </a>
                    <a href="{{ route('admin.commandes.index', ['status' => 'livre']) }}" class="px-4 py-2 rounded-full text-sm {{ request('status') === 'livre' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }}">
                        Livrées
                    </a>
                    <a href="{{ route('admin.commandes.index', ['status' => 'annule']) }}" class="px-4 py-2 rounded-full text-sm {{ request('status') === 'annule' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700' }}">
                        Annulées
                    </a>
                </div>
            </div>
                    ...
                    <table class="min-w-full bg-white">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="py-3 px-4 border-b border-gray-200">{{ $commande->stand->nom }}</td>
                                ...
                                <td class="py-3 px-4 border-b border-gray-200">
                                    <a href="{{ route('admin.commandes.show', $commande->id) }}" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-xs">Détails</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
