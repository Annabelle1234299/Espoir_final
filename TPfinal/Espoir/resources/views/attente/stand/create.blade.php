        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('attente.stand.store') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="emplacement" id="emplacement" value="{{ old('emplacement') }}" required
                                class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="type_produits" id="type_produits" required
                                ...
                                <option value="alimentaire" {{ old('type_produits') == 'alimentaire' ? 'selected' : '' }}>Produits alimentaires</option>
                                <option value="artisanat" {{ old('type_produits') == 'artisanat' ? 'selected' : '' }}>Artisanat</option>
                                <option value="vetements" {{ old('type_produits') == 'vetements' ? 'selected' : '' }}>Vêtements</option>
                                <option value="bijoux" {{ old('type_produits') == 'bijoux' ? 'selected' : '' }}>Bijoux</option>
                                <option value="cosmetiques" {{ old('type_produits') == 'cosmetiques' ? 'selected' : '' }}>Cosmétiques</option>
                                <option value="decoration" {{ old('type_produits') == 'decoration' ? 'selected' : '' }}>Décoration</option>
                                <option value="autre" {{ old('type_produits') == 'autre' ? 'selected' : '' }}>Autre</option>
                            </select>
                            @error('type_produits')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="infos_supplementaires" id="infos_supplementaires" rows="4"
                                class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('infos_supplementaires') }}</textarea>
